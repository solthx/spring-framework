...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			 ...
			 */
			prepareBeanFactory(beanFactory);  // 分析完毕

			try {
				// Allows post-processing of the bean factory in context subclasses.
				// 这个方法在当前版本的spring是没用任何代码的
				// 可能spring期待在后面的版本中去扩展吧
				postProcessBeanFactory(beanFactory); // 分析完毕

				// Invoke factory processors registered as beans in the context.
				// 做的主要事情就是：
				// 		对BeanDefinitionRegistryPostProcessors和BeanFactoryPostProcessor
				//		进行排序，执行！
				invokeBeanFactoryPostProcessors(beanFactory); // 分析完毕

				// Register bean processors that intercept bean creation.
				// 注册beanPostProcessor
				registerBeanPostProcessors(beanFactory); // 分析完毕

				// Initialize message source for this context.
				// 一些国际化配置，在不同地区的不同实现
				initMessageSource();  // 理解完毕

				// Initialize event multicaster for this context.
				//初始化应用事件广播器（ApplicationEventMulticaster）
				initApplicationEventMulticaster();  // 理解完毕

				// Initialize other special beans in specific context subclasses.
				// 钩子方法，初始化一些特殊的bean. （例如在web中会初始化UIXxx..
				onRefresh();  // 理解完毕

				// Check for listener beans and register them.
				// 把那些实现了ApplicationListener接口的Bean给注册到ApplicationEventMulticaster里！！
				registerListeners(); // 理解完毕

				// Instantiate all remaining (non-lazy-init) singletons.
				// 实例化所有剩余的（非延迟初始化）单例。
				finishBeanFactoryInitialization(beanFactory);  // 核心环节，认真去读
	...
	protected void prepareRefresh() {
		...
		// 检查标记为required的environment变量是否合法
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		refreshBeanFactory();  // 抽象方法, 在AbstractRefreshableApplicationContext中实现
		ConfigurableListableBeanFactory beanFactory = getBeanFactory(); // 抽象方法，实现位置同上
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		// 把ApplicationContext的类加载器传给内部容器BeanFactory，使得BeanFactory能够使用ApplicationContext的功能
		// Tell the internal bean factory to use the context's class loader etc.
		beanFactory.setBeanClassLoader(getClassLoader());
		//bean表达式解释器，能够获取bean当中的属性在前台页面（例如：bean01.attr1 就能够获取bean01的attr1属性）
		...
		// 能够在bean中获得到各种XxxAware（XxxAware都有其作用）
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));

		// 忽略这几个接口，spring容器就不会帮你注入这几个接口的实例
		// 以下几个类会在特殊场合进行注入，因此在这里就先忽略掉
		...
		// 设置几种类型的自动装配.
		// 例如：
		// 对于BeanFactory，就自动装配当前beanFactory变量（其实是DefaultListableBeanFactory）
		// 对于ResourceLoader，ApplicationEventPublisher，ApplicationContext，
		// 都自动装配this对象，即：AnnotatonConfigApplicationContext 或 XMLPath..ApplicationContext
		// 总之就是一个最顶层最强大的ApplicationContext！（ 因为这个ApplicationContext实现了这些接口嘛，所以自动状态的时候就装配它
		...
		// 注册一个后置处理器，主要检查是否实现了ApplicationListener接口，
		// 如果实现了就加入当前的applicationContext的applicationListeners列表
		beanFactory.addBeanPostProcessor(new ApplicationListenerDetector(this));

		// Detect a LoadTimeWeaver and prepare for weaving, if found.
		// 好像是AOP里对字节码的织入。。还没看到AOP，先放着，日后填坑
		// ToWrite
	...
	protected void initApplicationEventMulticaster() {
		...
		if (beanFactory.containsLocalBean(APPLICATION_EVENT_MULTICASTER_BEAN_NAME)) {
			...
			// 如果不存在，就手动创建. SimpleApplicationEventMulticaster就
			// 实现了ApplicationEventMulticaster接口
	...
	protected void registerListeners() {
		...
		// 获得实现了ApplicationListener接口的Bean，然后把他们给注册到EventMulticaster里！！！
